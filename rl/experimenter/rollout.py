...
class RO(object):
    ...
    def __init__(self, rollouts, max_n_rollouts=None, max_n_samples=None):
        ...
        self.obs, self.acs, self.eps, self.rws, self.sts, self.lps = self.flatten('all')
    ...
    def append(self, rollouts):
        ...
        self.obs, self.acs, self.eps, self.rws, self.sts, self.lps = self.flatten('all')
        print('RO has n_samples', self.n_samples, 'len', len(self))
    ...
    def flatten(self, field):
        ...
        if field == 'all':
            ...
            eps = self.flatten('eps')
            rws = self.flatten('rws')
            sts = self.flatten('sts_short')
            lps = self.flatten('lps')
            assert len(obs) == len(acs) == len(eps) == len(rws) == len(sts) == len(lps)
            return obs, acs, eps, rws, sts, lps
        else:
...
class Rollout(object):
    ...
    def __init__(self, obs, acs, eps, rws, sts, done, logp=None):
        # obs, acs, rws, sts, nos, lps are lists of vals
        # eps are the randomness in generating actions
        assert len(obs) == len(sts)
        assert len(acs) == len(rws)
        assert len(eps) == len(acs)
