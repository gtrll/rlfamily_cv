...
class MvpQuad(Quad):
    ...
    def primalmap(self, u):
        ...
        if self._cache is not None and self._use_cache:
            ratio = u / np.maximum(self._cache['u'], self._eps)  # avoid divided by zero
...
# TODO make Prox support addition and positive multiplication
# TODO negative entropy


# class BregmanDivergences(object):
#     # sum B_n(x||x_n)
#     # = sum  w_n ( R(x) - <dR(x_n), x-x_n> - R(x_n))
#     # = sum_w * R(x) - <sum_w_dR, x> + cst
#
#     def __init__(self, prox, x=None, w=1.0):
#         self.prox = prox
#         self._sum_w = w
#         self._sum_w_dR = w*self.prox.dualmap(x)
#
#     def proxstep(self, g):
#         #   argmin_x <g, x> + sum B_n(x||x_n)
#         # = argmin_x <g, x> + sum_w * R(x) - <sum_w_dR, x>
#         return self.prox.proxstep((g-self._sum_w_dR)/self._sum_w)
#
#     def __iadd__(self, other):
#         assert self.prox == other._prox
#         self._mum_w += other._sum_w
#         self._mum_w_dR += other._sum_w_dR
#         return self
#
