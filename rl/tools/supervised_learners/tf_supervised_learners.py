...
from rl.tools.utils.misc_utils import check_required_params
...
def _tfSupervisedLearnerDecorator(cls):
    ...
    class decorated_cls(cls, tfSupervisedLearner):
        ...
        def __init__(self,
                     # for tfFunctionApproximator
                     x_dim, y_dim, name=None, seed=None, build_nor=None, max_to_keep=None, pred_residue=False,
                     # for SupervisedLearner
                     use_aggregation=False, max_n_samples=None, max_n_update_samples=None,
                     batch_size_for_prediction=2048,
                     # for tfSupervisedLearner
                     batch_size=128, n_batches=2048, n_epochs=3, learning_rate=1e-3,
                     # other arguments for cls.__init__
                     **kwargs):
            ...
            tfSupervisedLearner.__init__(self, x_dim, y_dim, name=name, seed=seed,
                                         ...
                                         max_n_update_samples=max_n_update_samples,
                                         batch_size_for_prediction=batch_size_for_prediction)
            # build graph
            # NOTE this calls tfFunctionApproximator.__init__ again but it should be okay
            cls.__init__(self, x_dim, y_dim, name=name, seed=seed,
                         build_nor=build_nor, max_to_keep=max_to_keep, pred_residue=pred_residue, **kwargs)
            ...
            self.n_epochs = n_epochs  # maximal number of epochs over all the data
        ...
        def _update_func_approx(self, x, y, w, to_log=False, log_prefix=''):
            ...
            if to_log:
                ...
                logz.log_tabular('ExplainedVarianceBefore({}){}'.format(
        ...
        def _update_with_lr_search(self, x, y, w, decay=0.5, maxitr=1, reltol=1e-5):
            ...
            def update_via_minibatches(x, y, w, lr, batch_size, n_batches, n_epochs):
                n_batches_so_far = 0
                n_epochs_so_far = 0
                done = False
                while not done:
                    ...
                    # print('epoches: {}'.format(n_epochs_so_far))
                    n_epochs_so_far += 1
                    if n_epochs_so_far >= n_epochs:
                        done = True
            ...
            itr = 0
            while True:
                update_via_minibatches(x, y, w, lr, self.batch_size, self.n_batches, self.n_epochs)
